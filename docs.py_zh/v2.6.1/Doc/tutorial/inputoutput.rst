...
这种捷径更为简单，不过不提供完整的控制方法。两种选择用不同的方式管理行缓冲，不要把它们弄混了。
...
``f.tell()`` 返回一个整型值，给出文件对象指向的文件位置。从文件头算起，以字节为单
位。要改变文件对象的位置，可以用 ``f.seek(offset, from_what)``。从起点算起，移动
*offset* 个位置。这个起点由 *from_what* 参数决定。 * from_what* 为 0 时，从文件
头算起，1 则为文件对象当前的位置，2 就从文件尾算起。*from_what* 默认为 0，可以忽
略，此时从文件开头算起。
...
文件用完以后，调用 ``f.close()`` 关闭文件对象，释放占用的系统资源。调用
``f.close()`` 之后再使用这个文件对象就会出错。 ::
...
处理文件对象的时候使用 :keyword:`with` 关键字是个好办法。这样做的好处是即使中途
发生异常，文件用完后也会自动关闭。这其实是 :keyword:`try`\ -\ :keyword:`finally`
块的简写方式。 ::
...
文件对象还有一些不太常用的方法，例如 :meth:`isatty` 和 :meth:`truncate`。文件对
象的完整说明参见库参考手册。
...
字符串可以很方便的从文件中读写。数值稍微麻烦一点，因为 :meth:`read` 方法只返回字符串，需要
把获得的 ```'123'``` 之类的字符串用 :func:`int` 这样的函数转换为数值123。不过，如果
你想保存复杂数据类型，例如列表、字典或者类实例，这事儿就麻烦得多了。
...
Python 提供了一个名为 :mod:`pickle` 的标准模块，这样用户就不需要每次自己编写和调
试保存复杂数据类型的代码。这是一个令人赞叹的模块，它可以处理任何 Python 对象（甚
至是一些 python 代码！），将其序列化为一个字符串，这个过程称之为“封
装”（ :dfn:`unpickling` ），从字符串中重新构造出对象的过程称之为
拆封（ :dfn:`unpicking` ）。封装状态中的对象可以存储为文件或数据，也可以在网络上的不同机
器间传输。

If you have an object ``x``, and a file object ``f`` that's been opened for
writing, the simplest way to pickle the object takes only one line of code::

如果你有一个对象 ``x`` ，还有个已经打开，准备写入的文件对象 ``f`` 。最简单的对象
封装代码只需要一行::
...
（你不想把封装数据写入到文件，或者需要封装许多对象时，还有一些其它的变化，详见
Python 库参考手册的 :mod:`pickle` 部分。）
...
:mod:`pickle` 是 Python 存储对象以供其它程序或其本身调用的标准方法。提
供这一系列技术的是 :dfn:`perlsistent` 对象。因为 :mod:`pickle` 的应用非常广泛，
许多 Python 扩展的作者都非常注意类似矩阵这样的新数据类型是否适合封装和拆装。
